...
; wNumPCItems) and the session stash in SRAM (sMartSessionBag) are
...
.got_want
	...
	call OpenSRAM
	ld a, [sMartSessionBagCheck1]
	cp SAVE_CHECK_VALUE_1
	jr nz, .init_stash
	ld a, [sMartSessionBagCheck2]
	cp SAVE_CHECK_VALUE_2
	jr z, .have_stash
.init_stash
	farcall ResetMartSessionStash
.have_stash
	pop af
	ld b, a
...
; swap the live bag and the stash, byte for byte
	...
	ld hl, wTMsHMs
	ld de, sMartSessionBag
	ld bc, wNumPCItems - wTMsHMs
	call SwapBytes
	pop af
	ld [sMartSessionBagActive], a
	; A stash built before the list terminators existed leaves the
	; mart bag unable to hold items; rebuild it if so. This examines
	; only whichever side holds the mart bag, never the field bag.
	farcall SanitizeMartBag
	jmp CloseSRAM
...
; MT_MORTAR_SQUARE and TIN_TOWER_ROOF are outdoor maps within indoor maps.
; Dig and Escape Rope should not take you to them.
	ld a, [wPrevMapGroup]
	cp GROUP_MT_MORTAR_SQUARE
	jr nz, .not_mt_moon_square_or_tin_tower_roof
	assert GROUP_MT_MORTAR_SQUARE == GROUP_TIN_TOWER_ROOF
	ld a, [wPrevMapNumber]
	cp MAP_MT_MORTAR_SQUARE
	ret z
