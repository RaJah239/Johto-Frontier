BlackthornMart_MapEvents:
	def_warp_events
	warp_event  2,  7, BLACKTHORN_CITY, 5
	warp_event  3,  7, BLACKTHORN_CITY, 5

	def_coord_events

	def_bg_events

	def_object_events
	variable_mart_event 1, 3, PAL_NPC_BLACK
	object_event  7,  6, SPRITE_COOLTRAINER_M, SPRITEMOVEDATA_WALK_LEFT_RIGHT, 2, 0, -1, -1, 0, OBJECTTYPE_COMMAND, jumptextfaceplayer, BlackthornMartCooltrainerMText, -1
	object_event  5,  2, SPRITE_BLACK_BELT, SPRITEMOVEDATA_STANDING_DOWN, 0, 0, -1, -1, PAL_NPC_GREEN, OBJECTTYPE_COMMAND, jumptextfaceplayer, BlackthornMartBlackBeltText, -1

	object_const_def
	const BLACKTHORNMART_CLERK
	const BLACKTHORNMART_COOLTRAINER_M
	const BLACKTHORNMART_BLACK_BELT

BlackthornMart_MapScripts:
	def_scene_scripts

	def_callbacks

BlackthornMartCooltrainerMText:
	text "You can't buy Max"
	line "Revive, but it"
	cont "fully restores a"
	cont "fainted #mon."

	para "Beware--it won't"
	line "restore PP, the"
	cont "Power Points"
	cont "needed for moves."
	done

BlackthornMartBlackBeltText:
	text "Max Repel keeps"
	line "weak #mon away"
	cont "from you."

	para "It's the longest"
	line "lasting of the"
	cont "Repel sprays."

	para "There may be some-"
	line "thing else…"
	done
